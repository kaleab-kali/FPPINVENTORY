...
const ReturnApproval: React.FC = () => {
  ...
  const [selectedItem, setSelectedItem] = useState<Partial<UniqueItemInfo> | null>(null);
  const [form] = Form.useForm();

  const source = allUniqueItemsQuery.data
    ? allUniqueItemsQuery.data
         .filter((queryResult: UniqueItemInfo) => queryResult.approvalStatus === 'pending')
  ...
  const handleApprove = (condition: string) => {
    if (selectedItem) {
      console.log(`Approve purchase with ID: ${selectedItem.uniqueId}, Condition: ${condition}`);
      ...
      >
        <Form form={form} layout="vertical" name="form_in_modal">
          <Form.Item
            name="condition"
            label="Condition"
            rules={[{ required: true, message: 'Please select the condition of the item' }]}
          >
