...
const createINVStaff = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (existingINVStaff) {
      res
        .status(400)
        .json({ message: "Invetory Staff with this email already exists" });
      return;
    }
    ...
    res
      .status(201)
      .json({ message: "Invetory Staff created successfully", newINVStaff });
  } catch (error) {
    console.error(error);
...
const createAdmin = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    res
      .status(201)
      .json({ message: "Admin created successfully", newINVStaff });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Internal Server Error" });
  }
...
const loginINVStaff = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // Generate JWT token
    const role = invStaff.role;
    const token = jwt.sign(
      { id: invStaff._id, email, role: invStaff.role },
...
const updateINVStaff = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Inventory Staff updated successfully", invStaff });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Internal Server Error" });
  }
