...
const getAllUniqueItems = async (req: Request, res: Response): Promise<void> => {
  ...
  const getUniqueItemsByEmployeeId = async (req: Request, res: Response): Promise<void> => {
    try {
      const { employeeId } = req.params;
      const uniqueItems = await UniqueItem.find({ employeeId });
  
      if (!uniqueItems) {
        res.status(404).json({ message: "No unique items found for the specified employee ID." });
        return;
      }
